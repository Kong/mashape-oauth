...
describe('OAuth 1.0a', function () {
  ...
  describe('Performing Secure Request', function () {
    ...
    describe('Request with oauth_body_hash', function () {
      it('should body be ignored from generating Authorization.', function (done) {
        var oa = new OAuth({
          version: '1.0A',
          consumerKey: 'consumerKey',
          consumerSecret: RSAPrivateKey,
        ...
        oa.createClient = function (options) {
          oauthHeaders.push(options.headers.Authorization);
          return {
            write: function () {},
            on: function () {},
            end: function () {}
        ...
        //oauth_body_hash is generated by agreed algorism with host. eg. SHA1
        var request1 = oa.performSecureRequest({
          method: "POST",
          url: 'http://foo.com/blah',
          oauth_body_hash: 'oauth_body_hash',
          body: '<?xml version="1.0" encoding="UTF-8"?><localDate>0312</localDate>',
          type: 'application/xml; charset=UTF-8'
        });

        //without body
        var request2 = oa.performSecureRequest({
          method: "POST",
          url: 'http://foo.com/blah',
          oauth_body_hash: 'oauth_body_hash',
          type: 'application/xml; charset=UTF-8'
        ...
        //with other body format
        var request3 = oa.performSecureRequest({
          method: "POST",
          url: 'http://foo.com/blah',
          oauth_body_hash: 'oauth_body_hash',
          body: 'foo=1%2C2%2C3&bar=1%2B2',
          type: 'application/xml; charset=UTF-8'
        });

        assert.equal(oauthHeaders.length, 3);
        assert.equal(oauthHeaders[0], oauthHeaders[1]);
        assert.equal(oauthHeaders[0], oauthHeaders[2]);
        done();
      });
    });
  });
